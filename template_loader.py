...
class DebugLoader(BaseLoader):
    ...
    def get_contents(self, origin):
        try:
            ...
            raise TemplateDoesNotExist(origin, backend=self.engine)
             ...
             # Fallback for older django versions or for when loaders isn't iterable in expected way due to recursive wrapping
             # But find_template_loader usually returns a loader instance or list of them.
             # Actually, based on BaseLoader, custom loaders usually don't take `loaders` in init unless they are wrappers.
             # Let's align with Django's CachedLoader which wraps others.
             pass
    ...
    def debug_wrap(self, content, template_name):
        ...
        label_html = f'<div class="debug-label" style="{label_style}">{template_name}</div>'
